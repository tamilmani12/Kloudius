...
export default function Login() {
  ...
  setErrormsg('');
  const signupresult = await signup(name, email.toLowerCase(), password);
  ...
  return (
    ...
    <Text style={{textAlign:'center',fontSize:calculatedwidth(32),fontWeight:'bold',marginBottom:hs(10),color:'black'}}>Create Account</Text>
<Text style={{textAlign:'center',fontSize:calculatedwidth(16),color:'grey',marginBottom:hs(24)}}>Sign up to get started</Text>
    <View style={{paddingHorizontal:calculatedwidth(24)}}>
        <Text style={{fontSize:calculatedwidth(14),color:'black',fontWeight:'bold'}}>Name</Text>
        <TextInput value={name} onChangeText={(text)=>{setName(text)}} placeholder="Enter your name" style={{borderWidth:1,borderColor:errormsg?'red':'lightgrey',borderRadius:8,paddingHorizontal:calculatedwidth(12),paddingVertical:hs(10),marginTop:hs(8)}} />
        <Text style={{fontSize:calculatedwidth(14),color:'black',fontWeight:'bold',marginTop:hs(16)}}>Email Address</Text>
...
<TouchableOpacity onPress={()=>{setSecure(!secure)}}>
  ...
  />
        ...
        <TouchableOpacity onPress={handlesignup} style={{backgroundColor:'#F42525',paddingVertical:hs(12),borderRadius:8,marginTop:hs(24)}}>  
            <Text style={{color:'white',textAlign:'center',fontSize:calculatedwidth(16),fontWeight:'semibold'}}>Sign Up</Text>
        </TouchableOpacity>
         <View style={{flexDirection:'row',justifyContent:'center',marginTop:hs(16)}}>
            <Text style={{color:'grey',fontSize:calculatedwidth(14)}}>Already have an account? </Text>
            <TouchableOpacity onPress={()=>router.push('/login')}>
            <Text style={{color:'#F42525',fontSize:calculatedwidth(14),fontWeight:'bold'}}>Log In</Text>
